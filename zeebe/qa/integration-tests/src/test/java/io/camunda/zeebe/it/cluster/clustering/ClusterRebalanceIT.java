...
final class ClusterRebalanceIT {
  ...
  private static final String PROCESS_ID = "cluster-rebalance-it-process";
  ...
  void shouldRebalanceLeadershipAndResumeProcessing() {
    // given
    deployProcess();
    assertThat(createInstance()).isPositive();
  ...
  private HttpResponse<String> triggerRebalance() {
    final URI uri = cluster.availableGateway().restAddress().resolve(REBALANCE_PATH);
    ...
    Awaitility.await("the rebalance completes with at least one transferred partition")
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(body.hasNonNull("runningRebalance"))
  ...
  private void awaitCommandsAccepted() {
    Awaitility.await("a process instance can be created after the rebalance completes")
        .atMost(Duration.ofSeconds(30))
        .ignoreExceptions()
        .untilAsserted(() -> assertThat(createInstance()).isPositive());
  }
  ...
  private void deployProcess() {
    ...
    Awaitility.await("process deployed to the cluster")
